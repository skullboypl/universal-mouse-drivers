...
import {
  actionToKeyFun,
  keyFunToAction,
  BLITZ_ULTIMATE_BUTTONS,
} from './buttons'
import { BLITZ_ULTIMATE_IDENTITY } from './identity'
import {
  encodeMacroKeyWrite,
  macroKeyFunParams,
} from './macroFlash'
import {
  ...
  getBlitzAction,
  BLITZ_BUTTON_ACTIONS,
} from './actions'

/** Blitz dongle PID(s) for connection label. */
const DONGLE_PRODUCT_IDS = [0xf563]
...
export class BlitzUltimateDriver {
  ...
  constructor() {
    this._identity = BLITZ_ULTIMATE_IDENTITY
  ...
  setButtonAction(
    ...
    this.enqueueWrite(async () => {
      ...
      const actionDef = getBlitzAction(action)
      if (actionDef?.mediaUsage) {
        const [lo, hi] = actionDef.mediaUsage
        await this.trySendAll(
          encodeMediaShortcutWrite(button.flashIndex, lo, hi),
          'bulk',
        )
        await sleep(40)
      } else if (action !== 'macro' && action !== 'combo') {
        // Drop stale media/combo shortcut when switching to a plain KeyFun.
        await this.trySendAll(
          encodeClearShortcutWrite(button.flashIndex),
          'bulk',
        )
        await sleep(20)
      }
      ...
      if (this.transport?.kind === 'webhid') {
        ...
        this.lastVerifyNote = ok
          ? `Button ${buttonId} → ${action} verified`
  ...
  importProfile(json: string) {
    ...
    const templates = BLITZ_ULTIMATE_BUTTONS
    if (data.device && data.device !== BLITZ_ULTIMATE_IDENTITY.id) {
      throw new Error('Profile does not match Rampage Blitz Ultimate')
    }
